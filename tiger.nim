...
# S boxes, these ought be generated by the compiler.
let table1 = @[
...
const
  BlockSize* = 64
  ## Internal hash block size.

type
  Block* = array[BlockSize, char]
  ## Internal hash block type.
...
proc compress(state: var TigerState, data: var Block) =
  let data = cast[array[8, Word]](data)
  var
    a {.register.} = state.registers[0]
    b {.register.} = state.registers[1]
    c {.register.} = state.registers[2]
...
proc process*(state: var TigerState, data: openArray[char]) =
  ...
  if blockOff > 0:
    ...
    while blockOff < newOff:
      state.data[blockOff] = data[i]
      inc blockOff
      inc i
  if blockOff == BlockSize:
    state.compress(state.data)

  while (data.len - i) >= BlockSize:
    for j in 0..state.data.high:
      state.data[j] = data[i]
      inc i
    state.compress(state.data)
...
proc tiger*(data: string): TigerDigest =
  ## Generate a Tiger hash digest for a given string.
  var
    state: TigerState
    data = data
  init state
